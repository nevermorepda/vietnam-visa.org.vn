					...
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane active" id="for-tourist">
							<table id="export-list" class="table table-bordered table-hover">
								...
								<? $j=0; foreach ($items as $item) { 
									if ($item->visit_purpose == 'For tourist' && (!empty($item->fast_checkin) || !empty($item->car_pickup) || !empty($item->full_package))) {
								?>
								<tr class="row1 prss0">
									...
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<select name="note_fc" class="change-services change-services-<?=$item->book_id?>" pro-type="note_fc" item-id="<?=$item->book_id?>" style="border:none;text-align: center;width: 115px;">
										...
										<script type="text/javascript">
											$('.change-services-<?=$item->book_id?>').val('<?=$this->util->fc($item->full_package,$item->fast_checkin,$item->car_pickup,$item->booking_type_id)[0];?>');
										</script>
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<input type="text" class="note-input flight_number_fc" name="flight_number_fc" value="<?=!empty($item->flight_number_fc) ? $item->flight_number_fc : $item->vb_flight_number.' - '.$item->arrival_time;?>" pro-type="flight_number_fc" item-id="<?=$item->id?>" style="border:none;text-align: center;width: 170px;">
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<textarea style="border:none;text-align: center;" class="note-input" name="note_list_fc" pro-type="note_list_fc" item-id="<?=$item->id?>" rows="1"><?=!empty($item->note_list_fc) ? $item->note_list_fc : ''; ?></textarea>
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
								...
								<tr class="row1 prss0">
									<td style="vertical-align: middle;border-right: thin solid #999;"><?=$j+1?></td>
									<td style="vertical-align: middle;border-right: thin solid #999;" id="<?=$service->id?>">
									EX-<?=$service->id?>
									</td>
									<td style="vertical-align: middle;border-right: thin solid #999;">
										<input type="text" class="note-ex-input" name="welcome_name" value="<?=$service->welcome_name?>" pro-type="welcome_name" item-id="<?=$service->id?>" style="border:none;width: 150px;text-align: center;">
									</td>
									<td style="vertical-align: middle;border-right: thin solid #999;background: whitesmoke;" class="text-center">
									...
									<td style="vertical-align: middle;border-right: thin solid #999;" class="text-center">
										<input type="date" class="note-input-ex-arrivaldate note-input-ex-arrivaldate-<?=$service->id?>" name="arrival_date" value="<?=date('Y-m-d',strtotime($service->arrival_date))?>" pro-type="arrival_date" item-id="<?=$service->id?>" style="border:none;width: 120px;text-align: center;height:20px;">
									</td>
									<td style="vertical-align: middle;border-right: thin solid #999;" class="text-center">
										<select name="arrival_port" class="arrival_port note-input-ex-arrivalport note-input-ex-arrivalport-<?=$service->id?>" pro-type="arrival_port" item-id="<?=$service->id?>" style="border:none;width: 90px;text-align: center;height:20px;">
											<? foreach ($arrival_ports as $arrival_port) {
												?>
												<option value="<?=$arrival_port->short_name?>"><?=$arrival_port->short_name?></option>
											<?	
										...
										<script type="text/javascript">
											$('.note-input-ex-arrivalport-<?=$service->id?>').val('<?=$service->arrival_port?>');
										</script>
									</td>
									<td style="vertical-align: middle;border-right: thin solid #999;background: whitesmoke;" class="text-center"></td>
									<td style="vertical-align: middle;border-right: thin solid #999;background: whitesmoke;" class="text-center">
										
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;background: whitesmoke;" class="text-center" width="80px"></td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;background: whitesmoke;" class="text-center" width="100px">
										
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<select name="note_fc" class="change-ex-services change-ex-services-<?=$service->id?>" pro-type="note_fc" item-id="<?=$service->id?>" style="border:none;text-align: center;width: 115px;">
											<option value="1">Xe</option>
											<option value="2">Đón</option>
											<option value="3">Đón Vip</option>
											<option value="4">Đón + Xe</option>
											<option value="5">Đón Vip + Xe</option>
											<option value="6">Đón + Phí</option>
											<option value="7">Đón Vip + Phí</option>
											<option value="8">Đón + Phí + Xe</option>
											<option value="9">Đón Vip + Phí + Xe</option>
										</select>
										<script type="text/javascript">
											$('.change-ex-services-<?=$service->id?>').val('<?=$this->util->ex_fc($service->full_package,$service->fast_checkin,$service->car_pickup)[0];?>');
										</script>
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<input type="text" class="note-ex-input flight_number" name="flight_number" value="<?=!empty($service->flight_number) ? $service->flight_number : '';?>" pro-type="flight_number" item-id="<?=$service->id?>" style="border:none;text-align: center;width: 170px;">
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<textarea style="border:none;text-align: center;" class="note-ex-input" name="note" pro-type="note" item-id="<?=$service->id?>" rows="1"><?=!empty($service->note) ? $service->note : ''; ?></textarea>
									</td>
									<td rowspan="1" style="vertical-align: middle;border-right: thin solid #999;" class="text-center" width="100px">
										<label style="cursor: pointer;">
											<i class="fa fa-upload" data-toggle="modal" data-target="#myModal" typ='ex' pax_id="<?=$service->id?>"></i>
										</label>
									</td>
									<td style="vertical-align: middle;border-right: thin solid #999;">
										<div class="btn-group btn-processing-status">
											<a class="btn btn-xs dropdown-toggle dropdown-toggle-payment-ex-pax-<?=$service->id?>" data-toggle="dropdown">
												<? if($service->send_pickup == 1) { ?>
												<span class="label label-success">Send</span> <i class="fa fa-caret-down"></i>
												<? } else { ?>
												<span class="label label-danger " style="background:#777777">Sent</span> <i class="fa fa-caret-down"></i>
												<? } ?>
											...
											<ul class="dropdown-menu">
												<li>
													<a title="" class="payment-ex-pax" item-id="<?=$service->id?>" status-id="1"><span class="label label-success">Send</span></a>
													<a title="" class="payment-ex-pax" item-id="<?=$service->id?>" status-id="0"><span class="label label-danger " style="background:#777777">Sent</span></a>
												</li>
											</ul>
										</div>
									</td>
									<td style="vertical-align: middle;border-right: thin solid #999;">
										<div class="btn-group btn-processing-status">
											<a class="btn btn-xs dropdown-toggle dropdown-toggle-agents-pax-<?=$service->id?>" data-toggle="dropdown">
												<? $i=1; foreach ($agents as $agent) { 
													switch ($agent->id) {
														case 1:
															$label = 'success';
											...
											<ul class="dropdown-menu">
												...
												?>
												<li>
													<a title="" class="agents-ex-pax" item-id="<?=$service->id?>" agents-id="<?=$agent->id?>"><span class="label label-<?=$label?>"><?=$agent->name?></span></a>
												</li>
												<? $i++;} ?>
											</ul>
										</div>
						...
						<div role="tabpanel" class="tab-pane" id="for-business">
							<table id="export-list" class="table table-bordered table-hover">
								...
								<? $j=0; foreach ($items as $item) { 
									if ($item->visit_purpose == 'For business' && (!empty($item->fast_checkin) || !empty($item->car_pickup) || !empty($item->full_package))) {
								?>
								<tr class="row1 prss0">
									<td style="vertical-align: middle;border-right: thin solid #999;"><?=$j+1?></td>
									<td style="vertical-align: middle;border-right: thin solid #999;" id="<?=$item->pax_id?>">
...
<script type="text/javascript">
	$('.change-ex-services').change(function(event) {
		var val = $(this).val();
		...
		$.ajax({
			url : '<?=BASE_URL.'/syslog/ajax-change-ex-services'?>',
			type : 'POST',
			data : p,
	...
	$('.note-input-ex-arrivaldate').change(function(event) {
		var item_id = $(this).attr("item-id");
		...
		$.ajax({
			type: "POST",
			url: "<?=site_url("syslog/ajax-service-booking")?>",
			data: p,
	...
	$('.note-input-ex-arrivalport').change(function(event) {
		var item_id = $(this).attr("item-id");
		var pro_type = $(this).attr("pro-type");
		var val = $(this).val();
	...
	$(".note-ex-input").click(function() {
		$(this).select();
	});
	
	$(".note-ex-input").change(function() {
		var item_id = $(this).attr("item-id");
		var pro_type = $(this).attr("pro-type");
		var val = $(this).val();
		...
		$.ajax({
			type: "POST",
			url: "<?=site_url("syslog/ajax-visa-ex")?>",
			data: p,
			// success: function(result) {
			// 	console.log(result);
			// }
	...
	$(".payment-ex-pax").click(function() {
		...
		$.ajax({
			type: "POST",
			url: "<?=site_url("syslog/ajax-visa-ex-status")?>",
			data: p,
			success: function(result) {
				$(".dropdown-toggle-payment-ex-pax-" + item_id).html(status_label + " <i class=\"fa fa-caret-down\"></i>");
			}
		});
	});
	$(".agents-ex-pax").click(function() {
		var item_id = $(this).attr("item-id");
		...
		$.ajax({
			type: "POST",
			url: "<?=site_url("syslog/ajax-agents-ex-status")?>",
			data: p,
			success: function(result) {
				location.reload();
			}
...
<script type="text/javascript">
	...
	$('.fa-upload').click(function(event) {
		...
		var str_typ = null;
		if (typ != undefined) {
			$('#attach_file').attr('typ', 'ex');
			$('.btn-delete').attr('typ', 'ex');
			str_typ = 'ex';
		} else {
			$('#attach_file').removeAttr('typ');
			$('.btn-delete').removeAttr('typ');
		}
		var p = {};
		p['id'] = pax_id;
		p['str_typ'] = str_typ;
		$.ajax({
			url : '<?=BASE_URL.'/syslog/ajax-get-attach-file-fc'?>',
			type : 'POST',
			data : p,
			dataType: 'json',
			success : function(data) {
				if (data) {
					...
					var str = '';
					for (var i = 0; i < c; i++) {
						var src = '<?=BASE_URL.'/files/upload/image/passport_photo_fc/'?>'+pax_id+'/'+data[i];
						if (typ != undefined) {
							src = '<?=BASE_URL.'/files/upload/image/passport_photo_fc/ex/'?>'+pax_id+'/'+data[i];
						}
	...
	$(document).on('click', '.btn-delete', function(event) {
		...
		$.ajax({
			url : '<?=BASE_URL.'/syslog/ajax-delete-attach-file-fc'?>',
			type : 'POST',
			data : p
		});
	});
	$(document).on('change', '#attach_file', function(event) {
		...
		var pax_id = $(this).attr('pax_id');
		var ul = '<?=BASE_URL.'/syslog/ajax-attach-file-fc/'?>'+pax_id;
		if (typ != undefined) {
			ul += '/ex';
		}
		if (ins > 0) {
	...
	$(".payment-pax").click(function() {
		...
		$.ajax({
			type: "POST",
			url: "<?=site_url("syslog/ajax-visa-fc-pax-status")?>",
			data: p,
			success: function(result) {
