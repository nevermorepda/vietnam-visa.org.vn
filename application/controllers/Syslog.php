...
class Syslog extends CI_Controller {
	...
	public function processing_fees()
	{
		...
		$special_fees = $this->m_processing_special_fee->items();
		if (!empty($special_fees)) {
			$special_fee = array_shift($special_fees);
		} else {
			$special_fee = $this->m_processing_special_fee->instance();
		}
		
		$view_data = array();
		$view_data["breadcrumb"] = $this->_breadcrumb;
		$view_data["fee"] = $fee;
		$view_data["special_fee"] = $special_fee;
				...
				if ($booking->booking_type_id == 1) {
					$message = '<p>Dear <strong>'.$booking->contact_title.". ".$booking->contact_fullname.'</strong>,</p>
								...
								<ul style="list-style-type:decimal">
									...
									<li>Prepare 2 photos 4x6 cm (or same size passport photos). If you don’t have photos, you can have them taken at the arrival airport and it costs 5 USD/person in HCM (2 USD/person in Hanoi).</li>
									<li>Please click this link to download and fill in this Immigration form in advance to avoid wasting time at the airport: <a href="https://www.vietnam-visa.org.vn/files/upload/file/Documents/Form-XNC.pdf">https://www.vietnam-visa.org.vn/files/upload/file/Documents/Form-XNC.pdf</a>. This form will be dropped at the visa counter when you arrive, no need to send to Embassy.</li>
								</ul>
	...
	function ajax_special_processing_fee()
	{
		$visa_type = $this->input->post("visa_type");
		$val = $this->input->post("val");
		...
		if (empty($special_fees)) {
			$default = $this->m_processing_special_fee->instance();
			$data  = array(
				"tourist_1ms_urgent"		=> $default->tourist_1ms_urgent,
				"tourist_1ms_emergency"		=> $default->tourist_1ms_emergency,
				"tourist_1ms_holiday"		=> $default->tourist_1ms_holiday,
			...
			$this->m_processing_special_fee->add($data);
		}
