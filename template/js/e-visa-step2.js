	...
	$(".btn-next").click(function(){
		...
		for (var i=1; i<=applicants; i++) {
			...
			if ($("#passport_"+i).val() == "") {
				...
				$("#passport_"+i).removeClass("error");
			}
			if ($("#passport_type_"+i).val() == "") {
				$("#passport_type_"+i).addClass("error");
				err++;
				var txt = "Passport type is required.";
				if (msg.indexOf(txt) == -1) {
					msg.push(txt);
				}
			} else {
				$("#passport_type_"+i).removeClass("error");
			}
			if ($("#expirydate_"+i).val() == "") {
				$("#expirydate_"+i).addClass("error");
				err++;
				var txt = "Date of expiry is required.";
				if (msg.indexOf(txt) == -1) {
					msg.push(txt);
				}
			} else {
				$("#expirydate_"+i).removeClass("error");
			}
			if ($("#expirymonth_"+i).val() == "") {
				$("#expirymonth_"+i).addClass("error");
				err++;
				var txt = "Date of expiry is required.";
				if (msg.indexOf(txt) == -1) {
					msg.push(txt);
				...
				$("#expirymonth_"+i).removeClass("error");
			}
			if ($("#expiryyear_"+i).val() == "") {
				$("#expiryyear_"+i).addClass("error");
				err++;
				var txt = "Date of expiry is required.";
				if (msg.indexOf(txt) == -1) {
					msg.push(txt);
				...
				$("#expiryyear_"+i).removeClass("error");
			}
			if ($("#religion_"+i).val() == "") {
				$("#religion_"+i).addClass("error");
				err++;
				var txt = "Religion is required.";
				if (msg.indexOf(txt) == -1) {
					msg.push(txt);
				}
			} else {
				$("#religion_"+i).removeClass("error");
			}
			if ($(".photography-upload-"+i).val() == "") {
				var sts = parseInt($(".photography-upload-"+i).attr('sts'));
				if (sts == 0) {
					$(".file-photography-"+i).addClass("error");
					err++;
					var txt = "Passport photo type is required.";
					if (msg.indexOf(txt) == -1) {
						msg.push(txt);
					}
				}
			} else {
				$(".file-photography-"+i).removeClass("error");
			}
			if ($(".passport-upload-"+i).val() == "") {
				var sts = parseInt($(".passport-upload-"+i).attr('sts'));
				if (sts == 0) {
					$(".file-passport-"+i).addClass("error");
					err++;
					var txt = "Passport photo type is required.";
					if (msg.indexOf(txt) == -1) {
						msg.push(txt);
...
function calServiceFees()
{
	...
	$.ajax({
		...
		success: function(result) {
			private_fee			= parseInt(result[0][0]);
			package_checkin_fee	= parseInt(result[0][1]);
			checkin_fee			= parseInt(result[0][2]);
			car_fee				= parseInt(result[0][3]);
			service_fee			= parseInt(result[0][4]);
			rush_fee			= parseInt(result[0][5]);
			stamp_fee			= parseInt(result[0][6]);
			discount			= parseInt(result[0][7]);
			...
			$(".total_visa_price_t").html(result[1][0]);
			$(".service-fee-detail").html(result[1][1]);
		}
	});
	...
	$.ajax({
		type: "POST",
		url: BASE_URL + "/apply-e-visa/ajax-cal-service-fees-step2.html",
		data: p,
		dataType: "json",
		success: function(result) {
...
function _calServiceFees()
{
	...
	$.ajax({
		type: "POST",
		url: BASE_URL + "/apply-e-visa/ajax-cal-service-fees-detail.html",
		data: p,
		dataType: "html",
		success: function(result) {
			$(".service-fee-detail").html(result.toFixed(2));
		}
	});
}
